/*
  # Populate Sales Data for Customer Analytics

  1. Order Generation
    - Creates realistic order patterns for different customer segments
    - VIP customers: 20 orders, ₹800-2500 each
    - Loyal customers: 12 orders, ₹400-1200 each  
    - New customers: 3 orders, ₹300-800 each
    - Promising customers: 6 orders, ₹350-900 each
    - Regular customers: 2-6 orders, ₹250-700 each

  2. Customer Statistics Update
    - Updates total_visits, total_spent, last_visit
    - Calculates loyalty points (1 point per ₹10 spent)
    - Sets profile completion percentages
...
BEGIN
  -- Get a starting counter for order numbers
  SELECT COALESCE(MAX(EXTRACT(EPOCH FROM created_at)::bigint), 1000000) INTO order_counter
  FROM orders;
  ...
  LOOP
    FOR i IN 1..orders_per_customer LOOP
      ...
      order_number := order_prefix || '-' || LPAD(order_counter::text, 6, '0');
...
DECLARE
  ...
  order_status text;
  order_counter bigint;
  customer_hash bigint;
BEGIN
  -- Get a starting counter for order numbers
  SELECT COALESCE(MAX(EXTRACT(EPOCH FROM created_at)::bigint), 2000000) INTO order_counter
  FROM orders;
  ...
  LOOP
    ...
    -- Create a hash from the customer ID for uniqueness
    customer_hash := ABS(HASHTEXT(customer_record.id::text));
    
    FOR i IN 1..orders_count LOOP
      order_counter := order_counter + 1;
      order_amount := 250 + (RANDOM() * 450);
      order_date := NOW() - (RANDOM() * INTERVAL '90 days');
      order_number := 'REG-' || LPAD((order_counter + customer_hash + i)::text, 6, '0');
      payment_method := payment_methods[1 + (RANDOM() * array_length(payment_methods, 1))::int];
...
DECLARE
  ...
  campaign_types text[] := ARRAY['sms', 'email', 'whatsapp'];
...
BEGIN
  FOR restaurant_record IN SELECT id FROM restaurants LOOP
    FOR i IN 1..5 LOOP
      campaign_name := campaign_names[1 + (RANDOM() * array_length(campaign_names, 1))::int];
      campaign_type := campaign_types[1 + (RANDOM() * array_length(campaign_types, 1))::int];
      campaign_message := campaign_messages[1 + (RANDOM() * array_length(campaign_messages, 1))::int];
