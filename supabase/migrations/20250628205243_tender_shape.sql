/*
  ...
  1. Orders Creation
    - Generate realistic orders for different customer segments
    - VIP customers: 15-25 orders, ₹800-2500 per order
    - Loyal customers: 8-15 orders, ₹400-1200 per order
    - New customers: 1-4 orders, ₹300-800 per order
    - Promising customers: 4-8 orders, ₹350-900 per order
    - Regular customers: 2-6 orders, ₹250-700 per order

  2. Customer Statistics Update
    - Update total_visits, total_spent, last_visit
    - Calculate loyalty_points (1 point per ₹10 spent)
    - Update profile_completion based on available data

  3. Additional Data
    - Loyalty reward redemptions
    - Customer feedback entries
    - QR codes with scan counts
...
BEGIN
  ...
  LOOP
    ...
    FOR i IN 1..orders_count LOOP
      ...
      order_number := 'REG-' || LPAD((EXTRACT(EPOCH FROM NOW())::bigint + customer_record.id::bigint + i)::text, 6, '0');
      payment_method := payment_methods[1 + (RANDOM() * array_length(payment_methods, 1))::int];
...
DECLARE
  ...
  campaigns_info RECORD[] := ARRAY[
    ROW('Weekend Special Offer', 'sms', 'Join us this weekend for amazing discounts!'),
    ROW('New Menu Launch', 'email', 'Discover our exciting new menu items.'),
    ROW('Happy Hour Deal', 'whatsapp', 'Special prices on drinks from 5-7 PM!'),
    ROW('Birthday Special', 'sms', 'Celebrate your birthday with us - special treats await!'),
    ROW('Loyalty Rewards', 'email', 'Redeem your points for amazing rewards.'),
    ROW('Summer Special', 'sms', 'Beat the heat with our refreshing summer menu!'),
    ROW('Family Dinner Deal', 'email', 'Perfect family dining experience at great prices.'),
    ROW('Weekend Brunch', 'whatsapp', 'Join us for an amazing weekend brunch experience!'),
    ROW('Festival Celebration', 'sms', 'Celebrate the festival with special traditional dishes.'),
    ROW('Customer Appreciation', 'email', 'Thank you for being our valued customer!')
  ]::RECORD[];
  sent_count integer;
...
BEGIN
  FOR restaurant_record IN SELECT id FROM restaurants LOOP
    FOR i IN 1..5 LOOP
      campaign_data := campaigns_info[1 + (RANDOM() * array_length(campaigns_info, 1))::int];
