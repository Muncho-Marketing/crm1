/*
  # Generate Sample Data for Restaurant CRM

  1. New Data
    - Orders for different customer segments (VIP, Loyal, New, Promising)
    - Customer statistics updates based on orders
  ...
  2. Updates
    - Customer total visits, spend, and loyalty points
    - Profile completion percentages
    - Credits balance reflecting campaign usage

  3. Sample Data Categories
...
-- Create campaign history using individual inserts instead of record arrays
DO $$
DECLARE
  restaurant_record RECORD;
...
BEGIN
  FOR restaurant_record IN SELECT id FROM restaurants LOOP
    FOR i IN 1..5 LOOP
      campaign_idx := 1 + (RANDOM() * array_length(campaign_names, 1))::int;
      ...
      INSERT INTO campaigns (
        ...
        restaurant_record.id,
        campaign_names[campaign_idx],
        campaign_types[campaign_idx],
        'completed',
        campaign_messages[campaign_idx],
        '{"segments": ["all"]}',
